...
from urllib.parse import quote_plus
import traceback
from visual.app_visual import create_bar_chart, create_area_chart, create_line_chart
                ...
                try:
                    ...
                    if not extracted_query or extracted_query == "None":
                        ...
                        try:
                            ...
                            choose_column_prompt = f'''You are a data visualization expert who analyzes column names to determine the optimal X and Y axes for insightful visualizations.

                                                ```Column names to analyze: {columns_list}```

                                                INSTRUCTIONS:
                                                1. Carefully examine each column name in the provided list {columns_list}
                                                ...
                                                **Output format (always return JSON):**
                                                ```json
                                                {{
                                                "x_axis": "<VALID_X_AXIS_COLUMN>",
                                                "y_axis": "<VALID_Y_AXIS_COLUMN>"
                                                }}
                                                ```
                                            '''

                            identified_columns = llm.invoke(choose_column_prompt)
                            x_y_columns = json.loads(identified_columns.content)
                            ...
                            # Prepare chart data
                            try:
                             # Limit chart data to prevent browser performance issues
                                ...
                                with line_tab:
                                    if df.shape[1] <= 10:
                                        ...
                                        if fig:
                                            ...
                                            st.warning("Unable to create line chart with the current data.")
                                    else:
                                ...
                                with bar_tab:
                                    if df.shape[1] <= 10:
                                        ...
                                        if fig:
                                            ...
                                            st.warning("Unable to create bar chart with the current data.")
                                    else:
                                        st.warning("Dataframe contains too many columns for visualization.")
                                ...
                                with area_tab:
                                    if df.shape[1] <= 10:
                                        ...
                                        if fig:
                                            ...
                                            st.warning("Unable to create area chart with the current data.")
                                    else:
                                        st.warning("Dataframe contains too many columns for visualization.")
                                                
                            except Exception as chart_error:
                                with line_tab, bar_tab, area_tab:
